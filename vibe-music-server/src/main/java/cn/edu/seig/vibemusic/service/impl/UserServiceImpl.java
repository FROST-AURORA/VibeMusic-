...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public Result login(UserLoginDTO userLoginDTO) {
        ...
        if (DigestUtils.md5DigestAsHex(userLoginDTO.getPassword().getBytes()).equals(user.getPassword())) {
            ...
            claims.put(JwtClaimsConstant.ROLE, RoleEnum.USER.getRole());//角色权限
    ...
    public Result logout(String token) {
        // 去除 Bearer 前缀
        String pureToken = (token != null && token.startsWith("Bearer ")) ? token.substring(7) : token;
        // 注销token
        Boolean result = stringRedisTemplate.delete(pureToken);
     ...
     */
    @Override
    @Cacheable(key = "#userSearchDTO.pageNum + '-' + #userSearchDTO.pageSize + '-' + #userSearchDTO.username + '-' + #userSearchDTO.phone + '-' + #userSearchDTO.userStatus + '-admin'")
