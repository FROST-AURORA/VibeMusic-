...
import cn.edu.seig.vibemusic.constant.RsdisConstants;
...
public class ArtistServiceImpl extends ServiceImpl<ArtistMapper, Artist> implements IArtistService {
    ...
    public Result<PageResult<ArtistVO>> getAllArtists(ArtistDTO artistDTO) {
        // 分页查询 ID
        Page<Long> page = new Page<>(artistDTO.getPageNum(), artistDTO.getPageSize());
        IPage<Long> idPage = artistMapper.queryArtistIds(page, artistDTO.getArtistName(), artistDTO.getGender(), artistDTO.getArea());
        List<Long> artistIds = idPage.getRecords();

        if (artistIds.isEmpty()) {
            return Result.success(MessageConstant.DATA_NOT_FOUND, new PageResult<>(0L, null));
        }
        List<ArtistVO> artistVOList=cacheHelper.getArtistDetailsBatch(artistIds);

        return Result.success(new PageResult<>(idPage.getTotal(), artistVOList));
    }
    ...
    public Result<ArtistDetailVO> getArtistDetail(Long artistId, HttpServletRequest request) {
        ArtistDetailVO artistDetailVO = cacheHelper.getWithLock(
                RsdisConstants.DETAIL_SINGERIDS,
                artistId,
                ArtistDetailVO.class,
                artistMapper::getArtistDetailById
        );
